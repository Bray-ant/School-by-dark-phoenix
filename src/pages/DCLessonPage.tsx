...
export default function DCLessonPage() {
  const { chapterId, sectionId, lessonId } = useParams<{ chapterId: string; sectionId: string; lessonId: string }>();
    ...
    <div className="min-h-screen py-8 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-6 text-xs text-[#737373]">
          <Link to="/" className="hover:text-white flex items-center gap-1"><Home className="w-3 h-3" /></Link>
          <ChevronRight className="w-3 h-3" />
          <Link to={`/dc-circuit/${chapter.id}`} className="hover:text-white">{chapter.title}</Link>
          <ChevronRight className="w-3 h-3" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex items-center justify-between mt-8">
            {prevLesson ? (
              <Link to={`/dc-circuit/${chapter.id}/section/${prevLesson.sectionId}/lesson/${prevLesson.id}`} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs transition-colors">
                <ChevronLeft className="w-4 h-4" /> Previous
              </Link>
            ) : <div />}
            {nextLesson && (
              <Link to={`/dc-circuit/${chapter.id}/section/${nextLesson.sectionId}/lesson/${nextLesson.id}`} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#3b82f6] hover:bg-[#2563eb] text-white text-xs transition-colors">
