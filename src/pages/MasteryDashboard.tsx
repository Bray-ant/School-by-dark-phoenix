    ...
    <div className="min-h-screen py-8 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-2 mb-6 text-xs">
          <Link to="/" className="text-[#737373] hover:text-white flex items-center gap-1"><Home className="w-3 h-3" /></Link>
          <ChevronRight className="w-3 h-3 text-[#737373]" />
          <Link to="/practice" className="text-[#737373] hover:text-white flex items-center gap-1"><Dices className="w-3 h-3" /></Link>
          <ChevronRight className="w-3 h-3 text-[#737373]" />
            ...
            {weakAreas.length === 0 ? <p className="text-xs text-[#737373] py-4 text-center">No weak areas detected. Keep practicing!</p> : (
              <div className="space-y-2">{weakAreas.map(a => (
                <div key={a.topic} className="flex items-center gap-3 p-2.5 rounded-xl bg-[#ef4444]/5 border border-[#ef4444]/10">
                  ...
                  <Link to="/practice" onClick={() => sessionStorage.setItem('practiceTopic', a.topic)} className="px-2 py-1 rounded-lg bg-[#ef4444]/15 text-[#ef4444] text-[10px] font-medium hover:bg-[#ef4444]/25 transition-colors">Practice</Link>
                </div>
              ))}</div>
            )}
          </motion.div>
            ...
            {sortedStats.map(stat => (
              <div key={stat.topic} className="grid grid-cols-2 md:grid-cols-12 gap-2 items-center px-3 py-2.5 rounded-xl hover:bg-white/5 transition-colors">
                ...
                <div className="col-span-2 md:col-span-1 flex justify-end">
                  <Link to="/practice" onClick={() => sessionStorage.setItem('practiceTopic', stat.topic)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors" title={`Practice ${stat.topic}`}><ArrowUpRight className="w-3.5 h-3.5 text-[#737373]" /></Link>
                </div>
              </div>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-panel rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Link to="/practice" className="flex items-center gap-3 p-4 rounded-xl bg-[#ec4899]/5 border border-[#ec4899]/15 hover:bg-[#ec4899]/10 transition-colors group">
            ...
            <Link to="/exercises" className="flex items-center gap-3 p-4 rounded-xl bg-[#3b82f6]/5 border border-[#3b82f6]/15 hover:bg-[#3b82f6]/10 transition-colors group">
            ...
            <Link to="/" className="flex items-center gap-3 p-4 rounded-xl bg-[#10b981]/5 border border-[#10b981]/15 hover:bg-[#10b981]/10 transition-colors group">
